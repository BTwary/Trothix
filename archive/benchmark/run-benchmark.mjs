// Benchmark runner for the retired Pipeline C ("root legacy pipeline").
//
// *** STATUS: retired and archived — kept for historical reference only. ***
// This script imports root-level `core/router.js`, which no longer exists
// in this repository (it was removed as part of an earlier consolidation
// pass along with the rest of "Pipeline C" — see
// docs/trothix-architecture-audit.md section 1.3/4). Running it throws
// `ERR_MODULE_NOT_FOUND`. It is no longer wired into any npm script (the
// old `benchmark:legacy` script that invoked it has been removed from
// package.json) and does not affect CI or the production-relevant
// benchmark. The live, currently-used benchmark is
// `benchmark/run-benchmark-pipelineB.mjs` (via `npm run benchmark`), which
// runs against the same `benchmark/{nda,lease,tos}/*.txt` fixture documents
// but drives them through the real production engine (`Trothix.js`)
// instead of this file's `core/router.js` import. Moved to `archive/` and
// out of `benchmark/` so the active `benchmark/` directory only contains
// the live harness; all other documentation references to this script
// have been updated or marked historical in the same pass.
//
// Usage (historical — will not run):  node archive/benchmark/run-benchmark.mjs
//
