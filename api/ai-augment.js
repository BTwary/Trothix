// /api/ai-augment — server-side proxy to Gemini, Groq, Mistral, and OpenRouter.
...
import { isRateLimited as isGloballyRateLimited, isDistinctItemCountOverLimit } from "./_rateLimiter.js";
...
async function isIpRateLimited(ip, jobId) {
  // Sub-request cap: every request from this IP counts, same as the old
  // requestLog-based check (60/IP/min).
  const subRequestThrottled = await isGloballyRateLimited(`ai_augment_subreq_${ip}`, RATE_LIMIT_SUBREQUESTS, RATE_WINDOW_MS);
  if (subRequestThrottled) return true;

  // Distinct-job cap: only a NEW job ID from this IP grows the count, same
  // as the old jobLog Map-of-Maps check (6 distinct jobs/IP/min).
  return isDistinctItemCountOverLimit(`ai_augment_jobs_${ip}`, jobId, RATE_LIMIT_JOBS, RATE_WINDOW_MS);
}
...
export default async function handler(req, res) {
  ...
  if (await isIpRateLimited(ip, jobId)) {
  ...
  if (await isGloballyRateLimited("ai_augment_global", GLOBAL_LIMIT_PER_MIN, RATE_WINDOW_MS)) {
    await recordEvent("rate_limited");
    return res.status(429).json({
