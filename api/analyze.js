...
export default async function handler(req, res) {
  ...
  const { documentText, mode: rawMode, jobId: incomingJobId, chunkIndex, chunkCount, findings, userCountry, userRole, aiFallbackPayload } = req.body || {};
  ...
  if (userCountry && userRole && userCountry !== "Not specified" && userRole !== "Not specified") {
    const contextStr = `\n\nCONTEXT: The user is acting as a [${userRole}] in the country/jurisdiction of [${userCountry}]. Evaluate the document through this specific lens, focusing on how the terms impact this role under this jurisdiction's laws.`;
    systemPrompt += contextStr;
  }

  if (aiFallbackPayload) {
    // rawTextToAnalyze used to be generated by the local parser and then
    // discarded here — only the generic `context` string was ever sent,
    // so the model never saw the specific passage the parser flagged.
    // We surface the flagged excerpt(s) directly. We don't replace the
    // full document with just these snippets: for a "low_confidence"
    // flag (as opposed to a narrowly-scoped "missing one field" flag) the
    // model still needs the whole document to produce a complete report —
    // only *where to focus extra scrutiny* is the new information here.
    const reasons = aiFallbackPayload.map(r => r.context).filter(Boolean).join(' ');
    const snippets = [...new Set(aiFallbackPayload.map(r => r.rawTextToAnalyze).filter(Boolean))]
      .map((s, i) => `[Excerpt ${i + 1}]\n${s.slice(0, 1500)}`)
      .join('\n\n');
